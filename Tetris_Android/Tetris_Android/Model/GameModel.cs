...
using Tetris_Android.Shape_utils;
using Tetris_Android.Shape_events;
...
namespace Tetris_Android
{
    ...
    public class GameModel
    {
        ...
        public event EventHandler<DrawnEventArgs> Changed;
        public event EventHandler Stuck;
        public event EventHandler GameLost;

        public GameModel(IDataAccess _persistence, int width = (int)Tetris_Android.Size.SMALL)
        {
        ...
        public async Task SaveAsync(String path)
        {
            ...
            if (data.Count > 0)
            {
                await _persistence.SaveAsync(path, data.ToArray(), Size.X);
                data.Clear();
            }

        }

        public async Task LoadAsync(string path, int newSize = -1)
        {
            Shapes.Clear();
            rowComplete = new int[LENGTH];
            ...
            {
                if (newSize > 0)
                Size = new Coord(newSize, LENGTH);
                
                return;
            }

            String[] source = await _persistence.LoadAsync(path);
            ...
            {
                Size = new Coord((int)Tetris_Android.Size.SMALL, LENGTH);
